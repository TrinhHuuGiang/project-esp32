...
uint8_t gpio_74HC595_init_output_gate()
{
    ...
    if(spi_master_init_device_config(&(device_config), GPIO_74HC595_SPI_COMMAND_LEN,
        ...
        GPIO_74HC595_SPI_SCK_FREQ, GPIO_74HC595_LATCH_PIN, 0,0,0,0,0//<< flag 0 -> MSBit first
    )) 
    {
...
uint8_t gpio_74HC595_set_output_bit(uint32_t pins_order, uint32_t pins_value)
{
    ...
    // reverse reg befor send
    uint32_t rev_reg_state = reverse_bytes_32(temp_reg_state);
    ...
    // *tx_buffer_mod_ptr = rev_reg_state; <-- wrong because uint8_t will cast rev_reg_state
    memcpy(tx_buffer_mod_ptr, &rev_reg_state, GPIO_74HC595_SPI_TX_BIT_DATA_LEN/8);
